...
operatorTable = [
                 -- [ unaryPref "~"  negateForm  ] , --treated apart
                 [  
                   binaryInf "&"  ( :&: )  AssocLeft ,
                   binaryInf "|"  ( :|: )  AssocLeft
                ],   
...
negForm =
  try ( reservedOp "~" >>  negateForm <$> parens form  )  
   <|>
  try ( reserved "~" >>   negateForm <$> negForm )
  -- with reserveOp, double negation "~~ " (without spaces) is not correctly parsed
  <|>
  ( reservedOp "~" >>    negateForm <$> atomicForm )


negateForm :: Form Name -> Form Name
negateForm f = f :=>: FALSE
